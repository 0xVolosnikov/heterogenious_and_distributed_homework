...
//    cl::Buffer d_result(opencl.context, CL_MEM_READ_WRITE, n*sizeof(float));
    ...
    kernel.setArg(0, d_a);
//    cl::Buffer d_result2(opencl.context, CL_MEM_READ_WRITE, n*sizeof(float));
    kernel.setArg(1, d_a);
    kernel.setArg(2, 1*1024);
    opencl.queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(n/1024), cl::NDRange(1024));

    kernel.setArg(0, d_a);
//    cl::Buffer d_result3(opencl.context, CL_MEM_READ_WRITE, n*sizeof(float));
    kernel.setArg(1, d_a);
    ...
    //for (int k = 1024; k < n; k += 1024) {
    //  for (int p = 0; p < 1023; p++) {
    //     result[k + p] += result[k - 1];
    //  }
    //}
    auto t4 = clock_type::now();
